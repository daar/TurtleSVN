...
begin
  OpenDialog1.FileName:= edtFileNameLeft.Text;
  OpenDialog1.Filter:= AllFilesMask;
  if OpenDialog1.Execute then
  begin
    edtFileNameLeft.Text:= OpenDialog1.FileName;
    actReload.Execute;
  end;
end;
...
begin
  OpenDialog1.FileName:= edtFileNameRight.Text;
  OpenDialog1.Filter:= AllFilesMask;
  if OpenDialog1.Execute then
  begin
    edtFileNameRight.Text:= OpenDialog1.FileName;
    actReload.Execute;
  end;
end;
...
begin
  SaveDialog1.FileName:= edtFileNameLeft.FileName;
  if SaveDialog1.Execute then
  begin
    SaveToFile(SynDiffEditLeft, SaveDialog1.FileName);
    edtFileNameLeft.FileName:= SaveDialog1.FileName;
  end;
end;

procedure TfrmDiffer.actSaveRightAsExecute(Sender: TObject);
begin
  SaveDialog1.FileName:= edtFileNameRight.FileName;
  if SaveDialog1.Execute then
  begin
    SaveToFile(SynDiffEditLeft, SaveDialog1.FileName);
    edtFileNameRight.FileName:= SaveDialog1.FileName;
  end;
end;
...
var
  Message: String;
begin
  Message:= 'The two files are identical!' + LineEnding + LineEnding;
  Message+= edtFileNameLeft.Text + LineEnding + edtFileNameRight.Text;
  if MessageDlg('Differ', Message, mtWarning, [mbClose, mbCancel], 0, mbClose) = mrClose then
    Close
  else begin
...
begin
  try
    ...
    on EFOpenError do
    begin
      ShowMessage('Cannot open existing file: ' + FileName);
    end;
    on EReadError do
    begin
      ShowMessage('Error reading from file: ' + FileName);
    end;
  end;
end;
